...
def get_all_cell(gray_img, num_col = 4, min_cell_w= 20, min_cell_h = 20):
...
def check_tile_outside(dilate_test, bboxs_test):
    ...
    if dilate_test_cp.sum()/(dilate_test_cp.shape[0]*dilate_test_cp.shape[1])> 0.01:
...
def validate_cell(gray_origin, gray_file, i=0):
    img_origin = get_dilate(gray_origin)
    img_test = get_dilate(gray_file)
    img_test = cv2.resize(img_test,(img_origin.shape[1],img_origin.shape[0]))
...
#                                  bboxs_test['table'][0]:bboxs_test['table'][0]+bboxs_test['table'][2]]
    lst_location_cell_test = get_all_cell(table_test)
...
#                                  bboxs_origin['table'][0]:bboxs_origin['table'][0]+bboxs_origin['table'][2]]
    lst_location_cell_origin = get_all_cell(table_origin)
...
#         print(ratio_title)
        return False, "Gạch ở phần title", title_origin, title_test
...
#             results.append(["Lỗi gạch ở STT cell %s" %int((i-num_col)/step), ratio_cell, cell_origin, cell_test])
        ...
        x_origin, y_origin, w_origin, h_origin = lst_location_cell_origin[i+1]
        cell_origin = table_origin[y_origin:y_origin+h_origin, x_origin:x_origin+w_origin]
        ...
        elif abs(1 - ratio_cell) <= 0.119:
            results.append(["Không gạch", stt, ratio_cell, cell_origin, cell_test])
        else:# 0.2 > abs(1 - ratio_cell) > 0.07:
            results.append(["Gạch hợp lệ", stt, ratio_cell, cell_origin, cell_test])
